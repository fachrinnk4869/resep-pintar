...
class GenerateSparseVectors:
    def __init__(self, model_type: str = "splade", device: str = None):
        ...
        if self.model_type == "splade":
            ...
            raise ValueError("Unsupported model_type: choose 'splade', 'opensearch', or 'bm25'")
    ...
    def _init_opensearch(self):
        ...
        # Load IDF
        local_cached_path = hf_hub_download(repo_id=self.model_id, filename="idf.json")
        ...
        self.special_token_ids = [self.tokenizer.vocab[tok] for tok in self.tokenizer.special_tokens_map.values()]

    def _compute_opensearch(self, text: str) -> torch.Tensor:
        feature = self.tokenizer([text], padding=True, truncation=True, return_tensors='pt', return_token_type_ids=False).to(self.device)
        with torch.no_grad():
    ...
    def _init_bm25_vectorizer(self, texts: List[str]):
        self.vectorizer = TfidfVectorizer(analyzer="word", stop_words="english", max_features=30000, smooth_idf=True)
        self.vectorizer.fit(texts)

    # -------------------- PUBLIC API --------------------
    def get_sparse_vector(self, texts: Union[str, List[str]]) -> List[Dict[str, List[float]]]:
        """
        Return list of {'indices': [...], 'values': [...]} ready for Qdrant SparseVector
        """
        ...
        if self.model_type == "splade":
            ...
            for row in tfidf_matrix:
                results.append({"indices": row.indices.tolist(), "values": row.data.tolist()})
...
class GenerateDenseVector:
    def __init__(self):
        ...
        if not self.api_url:
            raise RuntimeError("SILICONFLOW_URL_EMBEDDING is not set in environment")
        if not self.api_key:
    ...
    def get_dense_embedding(self, text: str, dim_size: int = None) -> List[float]:
        ...
        try:
            ...
            if "embedding" not in data["data"][0]:
                raise ValueError("Field 'embedding' tidak ditemukan di 'data[0]'.")
