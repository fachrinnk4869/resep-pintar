...
def evaluate(db, dataset):
    ...
    for data in tqdm(dataset, desc="Evaluating dataset"):
        for query in data["query"]:
            ...
            dense_result, sparse_result = db.search_data(query, top_k=5)
            end_time = time.time()
    ...
    if total_score == 0:
        print("Warning: Total score is 0, no gold documents were found in the results.")
...
if __name__ == "__main__":
    ...
    for db in [
        ...
        score, avg_time = evaluate(
            elastic, dataset[:2])  # batasi 10 data dulu
